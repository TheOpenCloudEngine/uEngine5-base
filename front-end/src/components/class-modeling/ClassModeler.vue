<template xmlns:v-on="http://www.w3.org/1999/xhtml">
  ...
  >
    <template slot="toolbar-contents">
      <md-button v-if="id && !deployStatus" @click="generateRestService"><md-icon>flash_on</md-icon>Generate REST service</md-button>
      ...
      <div v-if="projectUrl">
        <div v-if="todoCount==0">
          ...
          <md-speed-dial md-open="hover" md-direction="down" class="md-fab-top-right" md-theme="light-blue">
            ...
            <md-button class="md-fab md-primary md-mini md-clean" @click="updateRestService({updateToCode: true, updateFromCode: true})">
              <md-tooltip md-direction="left">Sync with code</md-tooltip>
              <md-icon>repeat</md-icon>
            </md-button>

            <md-button class="md-fab md-primary md-mini md-clean" @click="updateRestService({updateToCode: true, updateFromCode: false})">
              <md-tooltip md-direction="left">Update to code</md-tooltip>
              <md-icon>skip_next</md-icon>
            </md-button>

            <md-button class="md-fab md-primary md-mini md-clean" @click="updateRestService({updateToCode: false, updateFromCode: true})">
        ...
        <md-button v-else><md-spinner :md-size="20" md-indeterminate class="md-accent"></md-spinner> working..{{todoCount}}</md-button>
      </div>
      ...
      generateRestService: function(options){
        ...
        me.$root.codi('create-app/' + me.id).save()
          ...
          function (response) {
            ...
            if(!message) try{message = response.data.message}catch(e){}

            if(!message) try{message = response.data}catch(e){}

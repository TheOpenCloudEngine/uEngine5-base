...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       * 트레이싱 태그로 액티비티를 찾는다.
       **/
      getActivityByTracingTag: function (tracingTag) {
        var me = this;
        var selected;
        var recursiveCheck = function (activity) {
          ...
          if (activity.tracingTag == tracingTag) {
            selected = activity;
          } else {
      ...
      /**
       * 트레이싱 태그로 부모 액티비티를 찾는다. 부모가 데피니션일 경우 null 리턴.
       **/
      getParentActivityByTracingTag: function (tracingTag) {
        var me = this;
        var selected;
      ...
      moveActivity: function (sourceTracingTag, targetTracingTag) {
        ...
        var parent;
        if (targetTracingTag) {
          parent = me.getActivityByTracingTag(targetTracingTag);
        }

        var activity = me.getActivityByTracingTag(sourceTracingTag);
        ...
        if (parent) {
          ...
          parent.childActivities[1].push(copy);
        } else {
          me.data.definition.childActivities[1].push(copy);
        }

        //이동 후 pre,next 연결선을 다시 그리도록 한다.

        //연결선 공식 만들기.

        me.bpmnVue.onUserAction();

        console.log('moved!!', sourceTracingTag, targetTracingTag, me.data.definition);
        return null;
      },
      /**
      ...
      validateDefinition: function (value) {
        ...
        var recursiveCheck = function (activity) {
          ...
          if (activity.childActivities && activity.childActivities.length) {

          }
        }
        $.each(definition.childActivities[1], function (i, activity) {
          recursiveCheck(activity);
        })

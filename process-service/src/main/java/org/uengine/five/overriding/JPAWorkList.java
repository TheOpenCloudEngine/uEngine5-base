    ...
    protected String addWorkItemImpl(String reservedTaskId, String userId, Map parameterMap, boolean isReservation, TransactionContext tc) throws RemoteException {
        ...
        try {
            ...
            if (reservedTaskId != null) {
                ...
                wl = worklistRepository.findById(taskId)
                        .orElse(null);
            } else {
...
//            }
    ...
    public void cancelWorkItem(String taskID, KeyedParameter[] options, TransactionContext tc)
        ...
        try {
            ...
            WorklistEntity wl = worklistRepository.findById(new Long(taskID)).orElse(null);
    ...
    public void updateWorkItem(
        ...
        try {
            ...
            WorklistEntity wlDAO = worklistRepository.findById(new Long(taskId)).orElse(null);
            ...
            for (int i = 0; i < parameters.length; i++) {
                ...
                } else if ("dispatchParam1".equals(parameter.getKey())) {
                    wlDAO.setDispatchParam1("" + parameter.getValue());
                } else if (KeyedParameter.DUEDATE.equals(parameter.getKey())) {
                    wlDAO.setDueDate((Date) parameter.getValue());
                } else if (KeyedParameter.DEFAULT_STATUS.equals(parameter.getKey())) {
                    wlDAO.setStatus((String) parameter.getValue());
                } else if ("endDate".equals(parameter.getKey())) {
                    wlDAO.setEndDate((Date) parameter.getValue());
                } else if ("saveDate".equals(parameter.getKey())) { //임시저장 시간 저장 (11.23)
